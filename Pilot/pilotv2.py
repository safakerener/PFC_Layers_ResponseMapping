...
def setupData(expInfo, dataDir=None):
    ...
    thisExp = data.ExperimentHandler(
        ...
        originPath='C:\\Users\\willi\\Desktop\\PFC_Layers_ResponseMapping\\Pilot\\pilotv2.py',
...
def run(expInfo, thisExp, win, inputs, globalClock=None, thisSession=None):
    ...
    # --- Initialize components for Routine "babyStim" ---
    ...
    response_map_2 = {
        ...
        '4': [text4_2, text8_2, num4_2],  # Add more entries as needed for additional text components
    }
    ...
    trialsFilepath = f'participantData/s{numParticipant}/block_{numSession}.xlsx'
    ...
    # set up handler to look after randomisation of conditions etc
    babySteps = data.TrialHandler(nReps=1.0, method='sequential', 
        extraInfo=expInfo, originPath=-1,
        trialList=data.importConditions('participantData/s15/block_0/babySteps_s15.xlsx'),
    ...
    for thisBabyStep in babySteps:
        ...
        for thisBabyloop in babyloop:
            ...
            # --- Prepare to start Routine "babyStim" ---
            continueRoutine = True
            # update component parameters for each repeat
            thisExp.addData('babyStim.started', globalClock.getTime())
            # keep track of which components have finished
            babyStimComponents = []
            for thisComponent in babyStimComponents:
                thisComponent.tStart = None
                thisComponent.tStop = None
                thisComponent.tStartRefresh = None
                thisComponent.tStopRefresh = None
            ...
            # --- Ending Routine "babyStim" ---
            for thisComponent in babyStimComponents:
                if hasattr(thisComponent, "setAutoDraw"):
                    thisComponent.setAutoDraw(False)
            thisExp.addData('babyStim.stopped', globalClock.getTime())
            # the Routine "babyStim" was not non-slip safe, so reset the non-slip timer
            routineTimer.reset()
        ...
        # keep track of which components have finished
        bResponseComponents = [fixCross_2, text1_2, text2_2, text3_2, text4_2, text5_2, text6_2, text7_2, text8_2, kb_2, num1_2, num2_2, num3_2, num4_2]
                ...
                if keys:
                    ...
                    for key, response_components_2 in response_map_2.items():
                        if key in keys and hit == 1:
                            fixCross.setColor([0, 1, 0], 'rgb')  # Set the color of the fixCross
                            for component in response_components_2:
                                component.setColor([0, 1, 0], 'rgb')  # Set the color of the fixCross
                        else:
                            fixCross.setColor([1, 0, 0], 'rgb')  # Set the color of the component
                            for component in response_components_2:
        ...
        if thisSession is not None:
            # if running in a Session with a Liaison client, send data up to now
            thisSession.sendExperimentData()
    # completed 1.0 repeats of 'babySteps'
